...
public class ViajeLogic implements ViajeLogica {
	...
	Date hoy = new Date(Calendar.YEAR,Calendar.MONTH,Calendar.DATE,Calendar.HOUR,Calendar.MINUTE,Calendar.SECOND);
    @Override
	...
	for (Trip trip : viajes) {
	    if(trip.getStatus().equals(TripStatus.OPEN) 
		    && trip.getMaxPax()!=0 && hoy.after(trip.getClosingDate()) )
	...
	for (Trip trip : listaViajes) {
	    ...
	    if(trip.getStatus().equals(TripStatus.CANCELLED) && trip.getStatus().equals(TripStatus.CLOSED))
